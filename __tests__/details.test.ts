...
describe("parseDetailData", () => {
  ...
  test("checks that multilpe charger types exist", () => {
    const charger: Charger = parseDetailData(input1);
    expect(charger).toEqual(output1);
  });

  test("test that when handed undefined input it will return undefined", () => {
    const charger: Charger = parseDetailData(undefined);
